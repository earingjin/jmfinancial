...
import { formatAssetProjectionOutlook } from '../../summary/SimpleSummaryReport';
      ...
      {calculable ? (
        <>
          ...
          {lumpSumEvents.length > 0 && (
            <>
              ...
              <table className="grade-table compact">
                <thead><tr><th>나이</th><th>지출 용도</th><th style={{ textAlign: 'right' }}>금액</th></tr></thead>
                <tbody>
                  {lumpSumEvents.map((ev) => (
                    <tr key={ev.key}>
                      <td className="num">{ev.age}세</td>
                      <td>{ev.name}</td>
                      <td className="num" style={{ textAlign: 'right' }}>{formatWon(ev.amount)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="report-assumption-box">
            <strong>계산 가정</strong>
            <span>연 단위 전망으로, 연초 자산에 예상수익률(연 {formatPercent(projection.assumedReturnRate)})을 적용한 뒤 해당 연도의 소득 · 생활비 · 목돈지출을 반영합니다.</span>
            <span>생활비는 연 {formatPercent(projection.inflationRate)}의 물가상승률을 복리로 적용합니다.</span>
            <span>{projection.lumpSumExpenseNote}</span>
            <span>실제 월별 자금 흐름과는 차이가 있을 수 있으며, 실제 물가 · 수익률 · 지출 변동에 따라 결과는 달라질 수 있습니다.</span>
          </div>
        </>
      ) : (
