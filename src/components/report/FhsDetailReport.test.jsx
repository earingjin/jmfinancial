...
function buildReportData({ withIncome }) {
  ...
  const { indicators, weakest, strongest, aggregates } = calcIndicators(input);
  const retirementLivingCost = 300;
  const enrichedIndicators = enrichIndicators({ indicators, weakest, strongest, aggregates, retirementLivingCost }).indicators;
  return {
...
describe('FhsDetailReport', () => {
  ...
  it('산출 가능한 지표는 게이지·구간표·참고 범위 문구를 함께 보여준다', () => {
    ...
    expect(html).toContain('월 총지출 대비 유동성자산 4~6개월분');
    expect(html).toContain('fhs-overall-conclusion-list');
    expect(html).toContain('점검 우선 지표');
    expect(html).toContain('안정적인 지표');
    expect(html).toContain(financialHealthInterpretation.conclusion);
