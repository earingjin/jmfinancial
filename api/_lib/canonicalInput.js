...
export function buildCanonicalInput(input) {
  ...
  if (result.assets.pensionAssetsInputMode !== 'simple') {
    ...
    result.assets.pensionAssets =
      n(pension.variableAnnuity) +
      n(pension.pensionSavingsAccount) +
      n(pension.irp) +
      n(pension.selfRetirementPension) +
      (result.basic?.hasSpouse ? n(pension.spouseRetirementPension) : 0) +
      pension.other;
  }
